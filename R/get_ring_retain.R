#' get_ring_retain
#' @description 随机获得新用户的环比系数：以[start,max]随机得到环比系数首项，后续各项(i)相比其前一项(i-1)均升高，升高幅度为lift[i]，而其下一项(i+1)相对自身(i)的升高程度(lift[i+1])小于等于lift(i)。以模拟留存随时间逐渐降低，但降低幅度越来越小的常态。
